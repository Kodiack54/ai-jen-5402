...
/**
 * Direct slug lookup - bypasses path detection when routing exists
 * Used when Chad has already determined project_slug from PC uploader routing
 */
function getProjectBySlug(slug) {
  if (!slug) return null;
  const normalizedSlug = slug.toLowerCase().trim();
  const project = projectBySlug[normalizedSlug];
...
async function autoCreateProject(folderName, fullPath, parentProject) {
  // Guard: never insert without a valid path
  if (fullPath === null || fullPath === undefined || typeof fullPath !== 'string' || fullPath.trim().length === 0) {
    logger.warn('Skipping project create - missing path', { folderName, fullPath });
    return null;
  }
  ...
  try {
    const { data, error } = await db.from('dev_projects').insert({
      name: name,
      slug: slug,
      server_path: normalizedPath,  // Always set (required column)
...
async function processSession(session) {
  ...
  // Trust routing from Chad if available (project_slug from PC uploader)
  let routedProjectInfo = null;
  if (session.project_slug) {
    routedProjectInfo = getProjectBySlug(session.project_slug);
    if (routedProjectInfo) {
      logger.info('Using routed project from Chad', { slug: session.project_slug, project_id: routedProjectInfo.project_id });
    }
  }
  ...
  const structureItems = await extractStructure(content, realProjectPath, routedProjectInfo);
  ...
  const smartItems = await extractWithClaude(content, realProjectPath, routedProjectInfo);
 ...
 */
async function extractStructure(content, sessionPath, routedProjectInfo = null) {
  const items = [];
  const projectInfo = routedProjectInfo || await detectProjectFromPath(sessionPath);
  ...
  for (const pattern of filePathPatterns) {
    ...
    for (const filePath of matches) {
      ...
      const pathProject = routedProjectInfo || await detectProjectFromPath(filePath) || projectInfo;
 ...
 */
async function extractWithClaude(content, sessionPath, routedProjectInfo = null) {
